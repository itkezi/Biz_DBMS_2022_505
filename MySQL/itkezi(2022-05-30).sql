...
-- mqSQL은 DB와 관련된 명령을 실행할 때
-- 가장 먼저 어떤 DB를 사용할 것인지 알려줘야 한다
USE myDB;

DROP TABLE tbl_test;

-- Table을 생성할 때 칼럼의 이름은 사전에 있는 키워드는 가급적 사용 지양
-- 사전에 있는 키워드를 사용할 때는 접두사를 붙여 사용하는 것이 좋다
...
MySQL을 사용하는데 MySQL에서 사용하는 방식이 DBMS의 업계 표준으로 정착

대부분의 DBMS 들은 먼저 DataBase를 생성하고
사용자가 DataBase에 접근할 수 있는 권한을 부여하고
사용자는 DataBase를 사용하겠다고 선언을 한 후
DB 관련된 명령을 사용한다
*/
...
SELECT * FROM mydb.tbl_test;

-- 다중데이터 INSERT하기
INSERT INTO tbl_test ( t_name )
VALUES ('김나연'), ('정하은'),('곽소연'),('김건'),('이수영');

SELECT * FROM tbl_test;
/*
tbl_test의 ID 칼럼은 AUTO_INCREMENT로 설정되었기 때문에
데이터를 INSERT 할 때마다 자동으로 squence 값이 생성되어 추가된다
5.x MySQL에서는 AUTO_INCREMENT 칼럼에 이슈가 있었다
5.x 숫자 type은 주로 INT형을 사용하는데 INT형은 정수 11자리이다
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 INSERT가 불가한 문제가 있었다
...
MEDIUMINT : 3Byte : 0 ~ 16777215 또는 -8388608 ~ 8388607 까지
...
PK아닌 경우는 반드시 NOT NULL과 UNIQUE를 같이 설정한다
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','이몽룡','컴공과',3,'010-111-1111','전라북도 익산시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0102','이소정','미술학과',4,'010-111-1112','광주광역시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0103','이명박','무용과',4,'010-111-1113','전라북도 익산시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0104','윤석열','무용과',2,'010-111-1114','서울특별시');
