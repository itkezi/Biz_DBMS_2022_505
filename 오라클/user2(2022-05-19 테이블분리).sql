...
하지만 현재 table에서 100개의 레코드(데이터들)가 변경되는 문제가 발생한다
한번에 다량의 레코드의 데이터가 변경되는 경우 데이터 무결성, transaction 등의 문제를 일으킬 수 있다
...
만약 과목명을 변경하려면 많은 데이터를 변경해야 하기 때문에 문제를 일으킬 것이다

tbl_score 테이블의 설계를 변경하여 과목정보 테이블을 생성하고 과목정보와 연계하여 과목명을 볼 수 있도록 하겠다
*/
...
CREATE(새로운 객체 생성), DROP(객체 제거), ALTER(객체의 구조변경)
...
-- 2. sub query를 사용하여
-- tbl_subject로부터 과목코드를 가져와서
-- tbl_score 테이블에 UPDATE 실행하기

-- 아래 명령은 전체 데이터의 sc_sbcode 칼럼의 값을 001 문자열로 대체한다
-- 드디어 일주일간 야근이다
-- xxx UPDATE와 DELETE 명령은 WHERE이 없을 때 절대 실행 금지 xxx
UPDATE tbl_score
SET sc_sbcode = '001';
...
-- 3. UPDATE 후 데이터 검증
-- tbl_score table의 과목명과 tbl_subject의 과목명이 일치하지 않아 UPDATE가 되지 않은 데이터가 있는지 확인
SELECT * 
FROM tbl_score
WHERE sc_sbcode = '001';

-- 만약 이전에 칼럼값을 001로 세팅하지 않았다면 IS NULL(값이 없다) 작성
SELECT * 
FROM tbl_score
...
-- 학번과 점수는 있는데 과목코드가 없으면 조회한 데이터가 문제가 있을 수 있다
-- 또한 점수를 입력하지 않은 것으로 생각하고 반복하여 값을 저장할 수 있다
-- 새로 생성한 칼럼을 NOT NULL로 설정한다
-- NOT NULL 제약 조건 : 값을 INSERT할 때 이 칼럼은 비워놓아서는 안된다
ALTER TABLE tbl_score MODIFY (sc_sbcode NOT NULL);
-- 이후부터 데이터를 INSERT 할 때 sc_sbcode 칼럼의 데이터가 없으면 INSERT가 거부된다
-- 데이터를 추가하고 나서 NOT NULL 설정해주기

-- 5. tbl_score 테이블에서 sc_subject(과목명) 칼럼은 필요가 없게 되었다
-- 조금이라도 저장 공간을 절약하기 위해 칼럼을 삭제하기
ALTER TABLE tbl_score DROP COLUMN sc_subject;

DESC tbl_score;

-- DBMS에서는 길이가 같은 문자열은 부둥호 조건으로 결과를 조회할 수 있다
SELECT * 
FROM tbl_score
...
/*
완전 JOIN
main table과 참조(foregn) table 간에 완전 참조 무결성이 유지되는 경우에만
결과에 신뢰성이 유지된다
...
-- (OUTTER) LEFT JOIN
-- main table과 참조 table간에 참조무결성이 유지되지 않는 경우 사용
-- main table은 (조건에 따라) 데이터를 전부 나열하고
-- 참조 table에 ON 조건에 맞는 데이터를 조회한다
-- 참조 table에 ON 조건에 맞는 데이터가 있으면 데이터를 가져오고 없으면 null 값을 가져온다

SELECT sc_stnum AS 학번, sc_sbcode AS 과목코드, sb_name AS 과목명, sc_score AS 점수
FROM tbl_score
    LEFT JOIN tbl_subject
        ON sc_sbcode = sb_code
-- WHERE sc_stnum <= 'S0003' main table 나열 조건 (없어도 됨)
ORDER BY sc_sbcode;
