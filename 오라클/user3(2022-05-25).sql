...
오라클 이외의 다른 DBMS 에서는 AUTO INCREMENT 라는 속성을 사용하여 데이터를 INSERT 할 때 자동으로 칼럼 데이터를 추가한다
하지만 오라클은 그러한 속성이 없어서 상당히 불편하다

오라클에서는 SEQUENCE라는 특별한 객체가 있어서 그 객체를 사용하여 같은 효과를 발휘한다

또는 PK에 해당하는 칼럼을 문자열 32byte 크기로 설정하고 UUID를 사용하기도 한다(Java, C, Python : 호스트 언어)와 함께 프로젝트를 할 때는 UUID를 사용한다
*/

-- seq_belong 이라는 SEQUENCE 객체를 생성하고 시작값을 1로, 자동 증가 옵션을 1로 설정하였다
...
-- SEQUENCE를 사용하여 자동 증가하는 SEQ를 만들고, INSERT 에서 사용하기
...
-- 20220001 학생이 소속된 학과가 D0001 인 것을 확인했다
...
-- 1. FK 설정에 앞서 참조 무결성 관계를 조회하기
-- 다음의 SQL의 결과에서 결과가 한 개도 없어야 참조 무결성 관계가 성립된다
...
-- 위의 SQL 결과 참조 무결성이 성립되지 않는 데이터들은 7의 b_seq를 갖고있다
-- 2. 참조 무결성이 성립되지 않은 데이터를 삭제한다
DELETE FROM tbl_belong WHERE b_seq IN(7);
...
-- 참조무결성 제약조건을 테이블에 추가하는데 이 때는 Relation 테이블에 추가한다

-- 테이블에 FK를 추가하여 참조무결성 제약 조건 부여
...
-- tbl_student에 아직 추가되지 않은 학번을 belong table 에 추가하기
-- tbl_student에 없는 20220004 학번을 추가하려고 했더니 parent not found 오류가 발생한다
INSERT INTO tbl_belong(b_seq, b_stnum, b_dcode)
...
-- child key found
-- FK를 설정할 때 parent table의 데이터를 삭제하려고 할 때 child table에 데이터 있으면 : 보통 삭제 금지
-- parent table 데이터의 변경(Update) 하려고 할 때 child table에 데이터 있으면 : 보통 변경 금지
